...
$serviceQuery = "SELECT s.*, u.first_name, u.last_name, u.tip_email, u.department 
...
if (!$service) {
    header("Location: dashboard.php#reports&error=service_not_found");
    exit();
}
...
$serviceImages = !empty($service['serviceImages']) ? explode(',', $service['serviceImages']) : [];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Review Reported Service - Admin</title>
    ...
    <div style="max-width: 1200px; margin: 20px auto; padding: 20px;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px;">
            ...
            <div style="background: white; padding: 20px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                <h2>Reported Service Details</h2>
                ...
                <div style="margin: 20px 0;">
                    <?php if (!empty($serviceImages)): ?>
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 10px;">
                            ...
                            ?>
                                <div style="width: 100%; height: 150px; background: #f5f5f5; display: flex; align-items: center; justify-content: center; border-radius: 8px; font-size: 0.8rem; color: #999;">
                                    Image not found
                                </div>
                            <?php
                                    endif;
                ...
                <div style="margin-top: 20px;">
                    <h3><?= htmlspecialchars($service['serviceTitle']) ?></h3>
                    <p style="font-size: 1.5rem; color: #007bff; font-weight: 600;">₱<?= number_format($service['startingPrice'], 2) ?>/hr</p>
                    
                    <div style="margin: 15px 0;">
                        <strong>Category:</strong> <?= ucwords(str_replace('_', ' ', $service['serviceCategory'])) ?><br>
                        <strong>Delivery Time:</strong> <?= ucwords(str_replace('_', ' ', $service['serviceDuration'])) ?><br>
                        <strong>Posted:</strong> <?= date('M d, Y', strtotime($service['posted_at'])) ?>
                    </div>

                    <div style="margin: 15px 0;">
                        <strong>Description:</strong>
                        <p><?= nl2br(htmlspecialchars($service['serviceDescription'])) ?></p>
                    </div>

                    <div style="margin: 15px 0; padding: 10px; background: #f8f9fa; border-radius: 6px;">
                        <strong>Provider Information:</strong><br>
                        Name: <?= htmlspecialchars($service['first_name'] . ' ' . $service['last_name']) ?><br>
                        Email: <?= htmlspecialchars($service['tip_email']) ?><br>
                        Department: <?= htmlspecialchars($service['department']) ?>
                    </div>
                </div>
            </div>
                ...
                <div style="background: white; padding: 20px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                    ...
                    <form action="handle_report_action.php" method="GET" style="margin-top: 20px;">
                        ...
                        <button type="submit" 
                                ...
                                onclick="return confirm('Send warning to provider?')">
                            ⚠️ Send Warning to Provider
                        </button>
                    </form>

                    <button onclick="if(confirm('⚠️ Delete this service permanently?')) window.location.href='handle_report_action.php?action=delete_item&report_id=<?= $report_id ?>';"
